...
type Computation () =
    ...
    static member Process sub (interval : float * float) f =
    ...
    static member GetIntervals delta f interval =
        interval |> Computation.Split 0.1 |> List.filter (fun x -> (x |> fst |> f) * (x |> snd |> f) <= 0.0) 
    ...
    static member BisectionMethod f interval (accuracy : float) =
        ...
        interval |> Computation.GetIntervals 0.1 f |> List.map (fun x -> sub 0 (fst x) (snd x))  
...
let getRes (a : float * int) b =  printfn "%A" (fst a, snd a, Math.Abs((fst a) - b))

printfn "Functon: y = x - 10sin(x)" 
printfn "First value is x, second value is count of steps"

printfn "Intervals: %A" (Computation.GetIntervals 0.11 f0 (-5.0, 3.0))

printfn "Bisection method" 
(List.map2 getRes (Computation.BisectionMethod f0 (-5.0, 3.0) 0.000001) rightResults) |> ignore

printfn "Secant method" 
(List.map2 getRes (Computation.SecantMethod f0 (-5.0, 3.0) 0.000001) rightResults) |> ignore

printfn "Newton method:"
(List.map2 getRes (Computation.NewtonMethod f0 derF0 (-5.0, 3.0) 0.000001) rightResults) |> ignore

printfn "Advanced newton method:" 
(List.map2 getRes (Computation.AdvncedNewtonMethod f0 derF0 (-5.0, 3.0) 0.000001) rightResults) |> ignore
